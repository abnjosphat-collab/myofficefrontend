  ...
  return (
    <div>
                    ...
                    <button type="button"
                      ...
                      {et ? (() => {
                        ...
                        <>
                          ...
                          <span className="text-[10px] font-bold leading-none tracking-wide text-amber-400">SBY</span>
      ...
      <div className={`flex flex-wrap items-center gap-x-4 gap-y-2 px-4 py-3 border-t ${t.border} text-[10px] ${t.textFaint}`}>
        ...
        <span className="flex items-center gap-1.5"><span className="w-7 h-7 rounded-lg bg-amber-500/[0.22] inline-flex items-center justify-center"><Shield className="h-3.5 w-3.5 text-amber-400" /></span>Standby</span>
      ...
      <div className="px-6 pb-6 space-y-4">
        ...
        {assignment.shift_type !== 'standby' && (assignment.standby_periods || []).length > 0 && (
          <div className="bg-amber-500/[0.05] rounded-xl overflow-hidden">
            <div className="flex items-center gap-2 px-3.5 py-2.5 border-b border-amber-500/15"><Shield className="h-3.5 w-3.5 text-amber-400" /><span className="text-xs font-semibold text-amber-400 uppercase tracking-wider">Standby Periods</span><span className="ml-auto text-xs text-amber-400/70">{(assignment.standby_periods || []).length} period{(assignment.standby_periods || []).length > 1 ? 's' : ''}</span></div>
            <div className="px-3.5 py-3 space-y-1.5">
              {(assignment.standby_periods || []).map((p, i) => <div key={i} className="flex items-center gap-2 text-sm text-amber-400/80"><span className="text-amber-500/50">·</span><span>{fmtDate(p.from)}</span><span className={t.textFaint}>→</span><span>{fmtDate(p.to)}</span></div>)}
            </div>
          </div>
        )}
        ...
        {form.shift_type !== 'standby' && (
          <div className="rounded-xl bg-amber-500/[0.05] overflow-hidden">
            <div className="flex items-center justify-between px-3.5 py-2.5 border-b border-amber-500/15">
              <div className="flex items-center gap-2"><Shield className="h-3.5 w-3.5 text-amber-400" /><span className="text-xs font-semibold text-amber-400 uppercase tracking-wider">Standby Periods</span></div>
              <button type="button" onClick={() => setForm(p => ({ ...p, standby_periods: [...p.standby_periods, { from: p.cycle_start_date, to: p.cycle_start_date }] }))} className="text-xs px-2 py-1 rounded-lg bg-amber-500/15 text-amber-400 hover:bg-amber-500/25 transition-all flex items-center gap-1">+ Add Period</button>
            </div>
