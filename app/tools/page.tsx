...
type Modal = { kind: ActionKind; tool?: Tool; employee?: Employee } | { kind: 'new'|'employee'|'auth'|'feedback'|'customize'|'import'|'export' } | { kind: 'edit'; tool: Tool } | { kind: 'attachments'; tool: Tool } | null;
const TOOLS_SESSION_KEY = 'myoffice.tools.session.v1';
const tabs = [{ value: 'register', label: 'Equipment', icon: 'box' }, { value: 'loans', label: 'In use', icon: 'out' }, { value: 'employees', label: 'Employees', icon: 'user' }, { value: 'activity', label: 'History', icon: 'history' }, { value: 'analytics', label: 'Analytics', icon: 'analytics', adminOnly: true }, { value: 'feedback', label: 'Feedback', icon: 'edit', adminOnly: true }] as const;
...
export default function ToolsPage() {
  ...
  async function reloadWorkspace(token = currentAccount?.token) {
    ...
    const [people,equipment,trail,changes]=await Promise.all([
  ...
  const visibleTabs = tabs.filter(item=>!('adminOnly' in item)||!item.adminOnly||currentAccount?.role==='admin');
  ...
  function chooseSearchResult(result:WorkspaceSearchResult) {
    ...
    if(target.type==='tab'){
      if((target.tab==='analytics'||target.tab==='feedback')&&currentAccount?.role!=='admin'){
