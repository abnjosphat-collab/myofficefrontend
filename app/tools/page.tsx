...
import { ToolsGuideCarousel } from './ToolsGuideCarousel';
...
export default function ToolsPage() {
  ...
  <ToolsDialog open={!!modal} onClose={()=>setModal(null)} wide={['customize','feedback','import','export'].includes(modal?.kind||'')} title={modal?.kind==='customize'?'Settings':modal?.kind==='new'?'Add equipment':modal?.kind==='employee'?'Add an employee':modal?.kind==='auth'?(currentAccount?'Your profile':'Workspace account'):modal?.kind==='feedback'?'Feedback & suggestions':modal?.kind==='import'?'Import a register':modal?.kind==='source-register'?'Upload a source register':modal?.kind==='export'?'Download this view':modal?.kind==='edit'?'Edit equipment details':modal?.kind==='attachments'?'Condition evidence':modal?.kind==='ready'?'Return to ready for use':modal?TITLES[modal.kind]:''} description={modal?.kind==='customize'?'Display and workspace preferences.':modal?.kind==='attachments'?'Keep photos and supporting documents with the item.':modal?.kind==='employee'?'Add a person to this workspace’s independent register.':modal?.kind==='auth'?(currentAccount?'Your identity and access for this workspace.':'Create an account or sign in. No email verification is required in this prototype.'):modal?.kind==='feedback'?'Tell us what would make the product clearer, faster or more useful.':modal?.kind==='import'?'Preview and validate Excel, CSV or Word records before adding them.':modal?.kind==='source-register'?'Permanently preserve the original file before updating the live database.':modal?.kind==='ready'?'Record what was repaired or checked before releasing the tool.':modal?.kind==='export'?'A clean copy of the current section and filters.':'Just the details needed for a clear record.'}>
