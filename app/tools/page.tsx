...
import { ToolsAuth } from './ToolsAuth';
...
export default function ToolsPage() {
  ...
  const undoRef = useRef<(redo?:boolean)=>Promise<void>>(async()=>{});
  ...
  async function reloadWorkspace(token = currentAccount?.token) {
    ...
    setUsage(analyticsData.usage.map(row=>({id:row.id,name:row.event,detail:row.detail,at:row.created_at})));
  ...
  const openAdminModal = (next: Exclude<Modal,null>) => { if (!currentAccount?.canIssue) { setModal({kind:'auth'}); toast.error('Sign in with an administrator account to manage equipment.'); return; } setSelectedId(null); setModal(next); };
  const openAction = (kind: ActionKind, tool?: Tool) => openAdminModal({ kind, tool });
  ...
  async function saveMovement(input: Movement) {
    ...
    try {
      ...
      await toolsApi.post(`/tools/${current.backendId}/commands`,requireToken(),{kind:input.kind,employee_id:employee?.backendId||employee?.employeeNumber,employee_name:employee?.name,department:input.department,location:input.location,expected_return_at:input.dueISO||input.due,job_reference:input.job,condition:input.condition,notes:input.notes,approval_ref:input.approvalRef,calibration:input.calibration,idempotency_key:crypto.randomUUID()});
      await reloadWorkspace(); setModal(null); setFeedback('Change saved. Undo is available.'); toast.success('Change saved');
  ...
  async function archiveTool(tool:Tool) {
    if (!currentAccount?.canIssue) { setSelectedId(null); setModal({kind:'auth'}); toast.error('An administrator account is required to archive equipment.'); return; }
  ...
  async function saveEmployee(employee:Employee) {
    try { await toolsApi.post('/employees',requireToken(),{employee_number:employee.employeeNumber,name:employee.name,department:employee.department,job_title:employee.jobTitle}); await reloadWorkspace(); setModal(null); setTab('employees'); toast.success(`${employee.name} added`); }
  ...
  <ToolsDialog open={!!selected} onClose={()=>setSelectedId(null)} title={selected?.name || 'Tool details'} description={selected ? `${selected.id} · ${selected.make}` : ''} wide>
    {selected && <><div className={s.detailLead}><ToolSymbol kind={selected.kind} imageUrl={primaryToolImage(selected)} name={selected.name}/><div><StatusLabel status={selected.status} archived={selected.archived}/><p>{selected.category} · {departmentOf(selected)}{selected.section ? ` / ${selected.section}` : ''}</p></div><Help label="Tool details">Current custody, condition and supporting records. Edit details without changing the tool ID. Use a handover to change custody.</Help></div>
      <div className={s.detailColumns}><div><dl className={s.facts}>{[['Location',selected.location],['Condition',selected.condition],['Serial number',selected.serial],['With',selected.holder || 'Tool room'],...(selected.holder ? [['Expected return',selected.due || '—'],...(selected.originalDue && selected.originalDue!==selected.due ? [['Original deadline',selected.originalDue]] : []),['Work order',selected.job || 'Not recorded']] : []),...(selected.calibration ? [['Calibration',selected.calibration]] : []),...(selected.approvalRef ? [['Register approval',selected.approvalRef]] : [])].map(([label,value])=><div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl>
      ...
      <div className={s.detailEvidence}><div className={s.fieldHeading}><strong>Condition evidence <small>{selected.evidence?.length || 0}</small></strong><button className={s.textButton} onClick={()=>openAdminModal({kind:'attachments',tool:selected})}><Icon name="attachment" size={15}/>Add / manage files</button></div>{selected.evidence?.length ? <EvidenceGallery files={selected.evidence}/> : <p className={s.formHint}>Optional photos or PDF records of the tool’s condition.</p>}</div>
      <div className={s.detailActions}>{!selected.archived && (selected.status==='available' ? <button className={s.primary} onClick={()=>openAction('issue',selected)}>Issue tool<Icon name="out"/></button> : selected.status!=='attention' ? <><button className={s.primary} onClick={()=>openAction('return',selected)}>Receive return<Icon name="back"/></button><button className={s.secondary} onClick={()=>openAction('transfer',selected)}><Icon name="swap"/>Transfer</button><button className={s.secondary} onClick={()=>openAction('extend',selected)}><Icon name="clock"/>Extend</button></> : null)}<button className={s.secondary} onClick={()=>openAdminModal({kind:'edit',tool:selected})}><Icon name="edit"/>Edit details</button><button className={s.textButton} disabled={!!selected.holder} title={selected.holder?'Receive this tool before archiving it.':'History is retained. You can undo this.'} onClick={()=>archiveTool(selected)}><Icon name={selected.archived?'reset':'archive'} size={17}/>{selected.archived?'Restore tool':'Archive'}</button></div>
    </>}
  </ToolsDialog>
