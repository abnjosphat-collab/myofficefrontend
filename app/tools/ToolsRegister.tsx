...
export function ToolsRegister({ tools, view, onSelect, onAction }: { tools: Tool[]; view: 'grid' | 'list'; onSelect: (id: string) => void; onAction: (kind: ActionKind, tool: Tool) => void }) {
  ...
  return <div className={s.grid}>{tools.map((tool, i) => <motion.article key={tool.id} className={s.card} initial={{ opacity: 0, y: reduced ? 0 : 8 }} animate={{ opacity: 1, y: 0 }} whileHover={reduced ? undefined : { y: -7, rotateX: 1.15, rotateY: -.7, scale: 1.009 }} transition={{ type: 'spring', stiffness: 330, damping: 25, opacity: { duration: .22, delay: reduced ? 0 : Math.min(i * .025, .15) } }}>
