...
export function MovementForm({ kind, initialTool, tools, employees = PEOPLE.map((label,index)=>({id:String(index),employeeNumber:label.split(' · ')[1],name:label.split(' · ')[0],department:'Engineering',active:true})), addFiles, onSave, onCancel }: { kind: ActionKind; initialTool?: Tool; tools: Tool[]; employees?: Employee[]; addFiles: AddEvidence; onSave: (input: Movement) => void; onCancel: () => void }) {
  const candidates = tools.filter(t => !t.archived && (kind === 'issue' ? t.status === 'available' : ['issued', 'overdue'].includes(t.status)));
  ...
  const [department, setDepartment] = useState(initialTool ? departmentOf(initialTool) : 'Engineering');
  const [error, setError] = useState('');
  const tool = candidates.find(t => `${t.id} · ${t.name}` === toolLabel);
  const employeeOptions = employees.filter(employee=>employee.active).map(employee=>`${employee.name} · ${employee.employeeNumber}`);
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    onSave({ kind, toolId: tool.id, person, location: location.trim(), due: displayDue, dueISO: due ? new Date(due).toISOString() : undefined, job: job.trim(), condition, notes: notes.trim(), evidence, calibration: kind === 'return' && needsCalibration ? calibration : undefined, gatePass, approvalRef, movementScope, department });
  }
  return <form onSubmit={handleSubmit} className={s.form}>
    ...
    <div className={s.formColumns}>
      ...
      {kind !== 'extend' && <div className={s.field}><div className={s.fieldHeading}><label htmlFor="tools-work-location">{kind === 'return' ? 'Return location' : 'Current work location'}</label><Help label="Location">Enter the exact place for this movement. Locations stay free text because teams and tools move frequently.</Help></div><input id="tools-work-location" aria-label={kind === 'return' ? 'Return location' : 'Current work location'} value={location} onChange={event=>setLocation(event.target.value)} placeholder="Workshop, plant area, shaft, office…" required /></div>}
    ...
    <p className={s.demoNote}>Prototype record. You can undo this change.</p>
  </form>;
}
...
export function EmployeeForm({ onSave, onCancel }: { onSave: (employee: Employee) => void; onCancel: () => void }) {
  const [name,setName]=useState(''); const [employeeNumber,setEmployeeNumber]=useState(''); const [department,setDepartment]=useState('Engineering'); const [jobTitle,setJobTitle]=useState('');
  return <form className={s.form} onSubmit={event=>{event.preventDefault();onSave({id:crypto.randomUUID(),name:name.trim(),employeeNumber:employeeNumber.trim(),department:department.trim(),jobTitle:jobTitle.trim(),active:true});}}><div className={s.formColumns}><div className={s.field}><label htmlFor="employee-name">Full name</label><input id="employee-name" aria-label="Full name" required value={name} onChange={event=>setName(event.target.value)} placeholder="Employee name"/></div><div className={s.field}><label htmlFor="employee-number">Employee number</label><input id="employee-number" aria-label="Employee number" required value={employeeNumber} onChange={event=>setEmployeeNumber(event.target.value)} placeholder="EMP-001"/></div><SuggestField label="Department" options={DEPARTMENTS} value={department} onChange={setDepartment}/><div className={s.field}><label htmlFor="employee-role">Job title <small>Optional</small></label><input id="employee-role" aria-label="Job title" value={jobTitle} onChange={event=>setJobTitle(event.target.value)} placeholder="Artisan, supervisor…"/></div></div><p className={s.formHint}>This employee register belongs only to the focused Tools workspace while sharing the wider ERP platform.</p><div className={s.formFooter}><button type="button" className={s.secondary} onClick={onCancel}>Cancel</button><button type="submit" className={s.primary}>Add employee<Icon name="plus" size={16}/></button></div></form>;
}
