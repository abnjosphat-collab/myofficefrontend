...
// ===== HELPER FUNCTIONS =====
const getPageConfig = (pageId: string): PageConfig => {
...
function VisualizationContent() {
  ...
  const loadPageData = async (pageId: string) => {
    ...
    try {
      ...
      const { pageData: mockData, firstChartKey } = generateMockPageData(pageId, config);
      setPageData(mockData);
      if (firstChartKey) {
        setActiveChartKey(firstChartKey);
      }

      // Load AI analysis
      loadAiAnalysis(config);
  ...
  const loadAiAnalysis = async (config: PageConfig) => {
    setAiLoading(true);
    try {
      // Mock AI analysis
      await new Promise(resolve => setTimeout(resolve, 1000));

      const mockAnalysis = generateMockAiAnalysis(config);
