...
import { useTheme, ACCENT, AccentIcon, useFontStyle, FONT_OPTIONS, PrimaryButton } from '@/components/shared/theme';
...
export function BottomBar({
  ...
  return (
    <div
      className={`fixed bottom-0 right-0 left-0 ${sidebarCollapsed ? 'lg:left-[76px]' : 'lg:left-64'} h-9 z-40 ${t.glass} border-x-0 border-b-0 ${t.design === 'dallaglio' ? '' : 'backdrop-saturate-150'} transition-[left] duration-300`}
      style={{ boxShadow: t.design === 'dallaglio' ? 'none' : (t.light ? '0 -8px 24px -16px rgba(15,23,42,0.18)' : '0 -8px 24px -14px rgba(0,0,0,0.5)') }}
    >
                ...
                >
                  {feedbackSent ? (
                    ...
                    <>
                      ...
                      />
                      <PrimaryButton icon={Send} fullWidth size="xs" accent="blue" className="mt-2" onClick={sendFeedback}>Send</PrimaryButton>
                    </>
                  )}
                </motion.div>
              </>
